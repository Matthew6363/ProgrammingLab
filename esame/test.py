...
def detect_similar_monthly_variations(time_series, years):
    ...
    if years[0] < int(time_series[0][0][:4]):
        raise ExamException('il primo anno fornito non è presente nella lista')
        
    #controllo che il secondo anno sia presente nella lista time_series
        
    if years[1] > int(time_series[-1][0][:4]):
        raise ExamException('il secondo anno fornito  non è presente nella lista ! ')
   ...
   #calcolo le variazioni del num dei pass per per il primo e il secondo anno forniti e li inserisco nelle apposite liste, inserisco 'mancante' se non la posso calcolare
    ...
    for i in range(11):
        ...
        if pass_primo_anno[i] == 'mancante' or pass_primo_anno[i+1] == 'mancante':
            variaz_mesi_primo_anno = 'mancante'
    ...
    for i in range(11):
        ...
        if pass_sec_anno[i] == 'mancante' or pass_sec_anno[i+1] == 'mancante':
            variaz_mesi_sec_anno = 'mancante'
    ...
    for i in range(11):
        ...
        if lista_variaz_primo_anno[i] == 'mancante' or lista_variaz_sec_anno[i] == 'mancante':

            lista_finale.append(False)


        elif lista_variaz_primo_anno[i] - lista_variaz_sec_anno[i] <-2 or lista_variaz_primo_anno[i] - lista_variaz_sec_anno[i] >2:
